...
```{r}
set.seed(1313)
randteam = as.vector(sample(sports$School,size=25))
randsport = as.vector(sample(sports$Sport,size=1))
sportsrd <- sports %>%
  filter(School %in% randteam) %>%
  filter(Sport %in% randsport)
ggplot(sportsrd, aes(x=Start.Year, y=Multi.Year.APR)) + 
  facet_wrap(~School+Sport,ncol=5) + 
  geom_point() + 
...
```{r}
model1 <- lmer(Multi.Year.APR ~ 1 + (1|Sport) + (1|School), 
               REML=T, data=sports)
