...
$Y_i,_j,_k = \alpha_0 +\beta_1APR_i,_j,_k + \beta_2Enrollment_i + \beta_3PercentWhite + \beta_4PercentCommunication_i + \beta_5NewEngland_i + \beta_6MidEast_i + \beta_7GreatLakes_i + \beta_8Plains_i + \beta_9SouthEast_i + \beta_10SouthWest_i + \beta_11RockyMountains_i + \beta_12FarWest_i + \beta_13Outlying_i ++ \epsilon_i,_j,_k$

```{r}
OLSfull = lm(Pct ~ centeredAPR + ugds + ugds_white + pcip09 + factor(Region), data = sports)
...
$Y_i,_j,_k = \alpha_0 + \beta_1APR_i,_j,_k  + \beta_2Enrollment_i + \beta_3PercentWhite + \beta_4PercentCommunication_i + \beta_5NewEngland_i + \beta_6MidEast_i + \beta_7GreatLakes_i + \beta_8Plains_i + \beta_9SouthEast_i + \beta_10SouthWest_i + \beta_11RockyMountains_i + \beta_12FarWest_i + \beta_13Outlying_i + s_i + p_i,_j + \epsilon_i,_j,_k$

```{r}
fullintercept = lmer(Pct ~ centeredAPR + ugds + ugds_white + pcip09 + factor(Region) + (1|School) + (1|program.id), REML=T, data=sports)
...
$Y_i,_j,_k = \alpha_0 +\beta_1APR_i,_j,_k + \beta_2Enrollment_i + \beta_3PercentWhite + \beta_4PercentCommunication_i + \beta_5NewEngland_i + \beta_6MidEast_i + \beta_7GreatLakes_i + \beta_8Plains_i + \beta_9SouthEast_i + \beta_10SouthWest_i + \beta_11RockyMountains_i + \beta_12FarWest_i + \beta_13Outlying_i + s_i + m_iAPR + p_i,_j + \epsilon_i,_j,_k$

```{r}
fullschoolslope = lmer(Pct ~ centeredAPR + ugds + ugds_white + pcip09 + factor(Region) + (centeredAPR|School) + (1|program.id), REML=T, data=sports)
...
$Y_i,_j,_k = \alpha_0 +\beta_1APR_i,_j,_k + \beta_2Enrollment_i + \beta_3PercentWhite + \beta_4PercentCommunication_i + \beta_5NewEngland_i + \beta_6MidEast_i + \beta_7GreatLakes_i + \beta_8Plains_i + \beta_9SouthEast_i + \beta_10SouthWest_i + \beta_11RockyMountains_i + \beta_12FarWest_i + \beta_13Outlying_i + s_i + p_i,_j + m_iAPR + \epsilon_i,_j,_k$

```{r}
fullprogramslope = lmer(Pct ~ centeredAPR + ugds + ugds_white + pcip09 + factor(Region) + (1|School) + (centeredAPR|program.id), REML=T, data=sports)
...
```{r figure1,fig.align="center",out.width="80%",fig.cap= 'Figure 1: Boxplots of Multi-Year APR scores for each sport. Plot (a) is based on all 17,700 observations from all 2,168 programs over all years, while plot (b) uses only one observation per program', echo=FALSE, warning=FALSE}
...
## Methodology
  ...
  Our main purpose is to interpret how program's APR scores, measuring academic success, impact program's win percentages.  Using Ordinary Least Squares regression and multiple mixed-effects models, we can evaluate this relationship.  However, it is important to include variables in our models that impact a program's win percentage outside of APR scores.  By looking at each variable from the College Scorecard Database and its correlation with APR scores and win percentage we can see if it is valuable to the models.  Figure 2 (below) explores this correlation between each variable, where a darker red indicates a stronger, negative correlation and a darker blue indicates a stronger positive correlation.  On the diagonal we see the large dark blue circles, representing that each variable is perfectly correlated with itself.  Looking at the row and column for win percentage in Figure 2, we can see that APR Scores and the total student enrollment are the most correlated with win percentage, the percent communication and percent male have very small correlation, and the average cost and average age have essentially no correlation.  With no correlation, the average cost and average age of enrollment will not be included in the models as they add no value to our results.
...
LMRegion <- ggplot(sports, aes(x = Pct, as.factor(Region))) +
  ...
  theme.1 +
  labs(x='Win Percentage', y='Geographic Region',title='The Difference in APR by Region')

LMRegion
```

  In the models used throughout our research, we will explore the effect inclusion of these correlated variables have to our results.  One area of concern arises when considering the inclusion of the percent white variable.  Although this variable is correlated with win percentage, it is also highly correlated with the APR scores and may cause an issue of multicollinearity; the two variables may not be independent from each other and the models cannot differentiate the effect of APR scores from the effect of the percentage of white students.  Multicollinearity can cause variations in coefficient estimates and in the significance of our results.  We will include models that include these variables, and models that do not to determine the effect multicollinearity may have based on the differences in our results.

  To begin with we can look at an Ordinary Least Squares (OLS) regression of Multi-Year APR scores on Win Percentages.  Equation 1 below is the full equation used in our research to calculate the OLS regression coefficients.  $Y_i,_j,_k$ represents the Win Percentage of program $j$ at school $i$ in year $k$.  The $\beta_s$ in this model is the regression coefficient corresponding to each variable in the model.  $\epsilon_i,_j,_k$ then represents the error term capturing the unexplained variability in the model; the random differences within a specific program from year to year.
  
  $Equation 1: Y_i,_j,_k = \alpha_0 +\beta_1APR_i,_j,_k + \beta_2Enrollment_i + \beta_3PercentWhite + \beta_4PercentCommunication_i + \beta_5NewEngland_i + \beta_6MidEast_i + \beta_7GreatLakes_i + \beta_8Plains_i + \beta_9SouthEast_i + \beta_10SouthWest_i + \beta_11RockyMountains_i + \beta_12FarWest_i + \beta_13Outlying_i + \epsilon_i,_j,_k$

  However, the statistical modeling used in this research is a three-level mixed effects model.  We can compare our results to a traditional ordinary least squares (OLS) linear regression model however our data violates a vital assumption of OLS models: that all entries in the data are independent.  There are three levels in our model in which the entries have the potential to be correlated to one another: the school or institution level, the level containing each specific program, and the year level.  The key difference between an OLS model and a mixed effects model is the inclusion of random effects.  These random variables control for the random variability at each level in our model.
  
  The first level, the schools, are very important to consider.  Any entries from a specific college or university are going to be correlated with other entries from the same school.  For example, the win percentage of any program at the University of Michigan is going to be correlated with the win percentage at the University of Michigan in other entries, therefore not all entries are independent.  
  
  The second level in our model is the program level.  In this level it is important to take note that the win percentage of a specific program in one year is going to be correlated with other entries from the same program.  The University of Alabama historically has a very successful football program, and we can expect that the win percentage of the University of Alabama in 2010 is not independent from its win percentage in 2011.  
  
  The third and final level in this model is the year.  For this level we  need to consider that for any one program at any specific school, there will be random difference from year to year.
