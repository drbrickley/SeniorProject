...
table1 <- sports %>% select(School, Region, "Percent Communication Major" = pcip09, "Total Enrollment" = ugds, "Percent Male" = ugds_men, "Percent White" = ugds_white, "Average Cost of Attendance" = costt4_a, "Average Age at Enrollment" = age_entry, Sport, "Win Percentage" = Pct, "Multi-Year APR" = Multi.Year.APR, "Year" = Start.Year)
preview <- head(table1, 5)
stargazer(preview, type = 'html', summary = FALSE, title= "Table 1: Selected variables from the first five observations in the dataset.")
```
...
stargazer(summarydata, type = "html", title = "Table 2: Summary Statistics", summary.stat = c("mean","sd", "min", "p25", "median", "p75", "max"), digits = 1, digits.extra = 0)
...
$$
  ...
  When using a linear mixed effects model, there are both fixed effects and random effects.  In our models, the $\beta_s$ represent the fixed effects.  These variables will be constant over the program or school it represents.  In our mixed effect models the random effects, that vary over each program or school, will be represented by $s_i$, $p_{ij}$, $m_i$, $m_{ij}$ and $\epsilon_i,_j,_k$.  
...
- $\alpha_0 = 38.572 =$ the mean win percentage of an athletic program with a mean APR Score of 974.4 at a U.S. service school where enrollment is 0 students, 0 percent of which are white, 0 percent of which are male, and 0 percent are communication majors.  Of course there is no school in our data where this holds, so this estimate is not a valid interpretation of any school in our dataset.

- $\beta_1 = 0.104 =$ the estimated increase in a program's win percentage for a 1 point increase in APR score after controlling for total enrollment, the percent white, the percent male, the percent communications majors, and the geographic region.  With a T-Statistic of 10.544, this is statistically significant.  

- $\beta_2 = 0.000267 =$ the estimated increase in win percentage for one more enrolled student at the school after controlling for APR score, the percent white, the percent male, the percent communications majors, and the geographic region.  A single student increase is not reasonable, so we can say that a 1,000 student increase in enrollment corresponds to 0.267 percentage point increase in win percentage.  With a T-Statistic of 7.905, this is also statistically significant.

- $\beta_3 = 0.118 =$ the estimated increase in win percentage for a 1 percentage point increase in the percentage of the student body that is white after controlling for APR score, total enrollment, the percent male, the percent communications majors, and the geographic region.  The T-Statistic of 7.494 shows that this is significant.

- $\beta_4 = 0.011 =$ the estimated increase in win percentage for a 1 percentage point increase in the percentage of the student body that is male after controlling for APR score, total enrollment, the percent white, the percent communications majors, and the geographic region. The small T-Statistic of 0.263 shows this is insignificant.

- $\beta_5 = 0.226 =$ the estimated increase in win percentage for a 1 percentage point increase in the percentage of the student body that is majoring in communications or a related field after controlling for APR score, total enrollment, the percent white, the percent male, and the geographic region. The T-Statistic of 3.213 is significant, but not as much as APR Score, Enrollment, or Percent White.  

- ${\beta}_{6} = -3.744 =$ the estimated decrease in win percentage compared to U.S. service schools for schools that are in the states of Connecticut, Maine, Massachusetts, New Hampshire, Rhode Island, and Vermont. The low T-Statistic demonstrates that this is statistically insignificant.

- ${\beta}_{7} = -3.618 =$ the estimated decrease in win percentage compared to U.S. service schools for schools that are in the states of Delaware, District of Columbia, Maryland, New Jersey, New York, and Pennsylvania. The low T-Statistic demonstrates that this is statistically insignificant.

- ${\beta}_{8} = -3.528 =$ the estimated decrease in win percentage compared to U.S. service schools for schools that are in the states of Illinois, Indiana, Michigan, Ohio, and Wisconsin. The low T-Statistic demonstrates that this is statistically insignificant.

- ${\beta}_{9} = -0.444 =$ the estimated decrease in win percentage compared to U.S. service schools for schools that are in the states of Iowa, Kansas, Minnesota, Missouri, Nevada, North Dakota, and South Dakota. The low T-Statistic demonstrates that this is statistically insignificant.

- ${\beta}_{10} = 0.560 =$ the estimated decrease in win percentage compared to U.S. service schools for schools that are in the states of Alabama, Arkansas, Florida, Georgia, Kentucky, Louisiana, Mississippi, North Carolina, South Carolina, Tennessee, Virginia, and West Virginia. The low T-Statistic demonstrates that this is statistically insignificant.

- ${\beta}_{11} = 1.978 =$ the estimated decrease in win percentage compared to U.S. service schools for schools that are in the states of Arizona, New Mexico, Oklahoma, and Texas. The low T-Statistic demonstrates that this is statistically insignificant.

- ${\beta}_{12} = -1.894 =$ the estimated decrease in win percentage compared to U.S. service schools for schools that are in the states of Colorado, Idaho, Montana, Utah, and Wyoming. The low T-Statistic demonstrates that this is statistically insignificant.

- ${\beta}_{13} = -1.626 =$ the estimated decrease in win percentage compared to U.S. service schools for schools that are in the states of Alaska, California, Hawaii, Nevada, Oregon, and Washington. The low T-Statistic demonstrates that this is statistically insignificant.
...
- $\sigma_m = \sqrt0.052 = 0.228 =$ the standard deviation in rates of change in win percentage relative to APR scores.

- $\rho_{01} = 0.145 =$ the correlation in programs’ random intercept and their random rates of change.  This means that programs with an above average APR score see sharper increases in Win Percentage on average, however this is a small correlation.  
...
Transitioning from the OLS model and comparing various mixed effects models using AIC, BIC, and likelihood ratio tests allow our final model to yield the most accurate results.  Using our final mixed effects model of 2,168 different programs over 8 years, we see the direct positive relationship between academic success, measured through APR scores, and program's win percentage, a measure of athletic performance.  Of the numerous covariate factors included in this model, academics have the most significant effect on athletics.  The NCAA and all college coaches and recruiters for team sports should be looking not just for the best athletes, but for the best student-athletes.  In addition, the NCAA and college programs should seek to provide incentives to programs with higher academic performance, as it will benefit the competition at the college-level and create better students, better athletes, and in general people who are better prepared for their life after college.
...
```{r table3, echo=FALSE, out.width = "60%", warning=FALSE, results = 'asis'}
`Model 1` <- OLSinitial
`Model 2` <- OLSpartial
`Model 3` <- OLSfull

stargazer(`Model 1`, `Model 2`, `Model 3`, title = "OLS Results", align = TRUE, type = 'html',  dep.var.labels= "Win Percentage", covariate.labels = c("Centered APR Score", "Total Enrollment", "Percent White", "Percent Male", "Percent Communication Majors", "New England", "Mid East", "Great Lakes", "Plains", "Southeast", "Southwest", "Rocky Mountains", "Far West"), column.labels = c("Model 1", "Model 2", "Model 3"))
```