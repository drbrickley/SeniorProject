...
$Y_i,_j,_k = \beta_0 +\beta_1APR_i,_j,_k + \epsilon_i,_j,_k$

```{r}
OLSmodel = lm(Pct ~ centeredAPR, data = sports)

coef(summary(OLSmodel))
```

$Y_i,_j,_k = \beta_0 +\beta_1APR_i,_j,_k + s_i + p_i,_j + \epsilon_i,_j,_k$

```{r}
fullmodel = lmer(Pct ~ centeredAPR + (1|School) + (1|program.id), REML=T, data=sports)
...
$Y_i,_j,_k = \beta_0 +\beta_1APR_i,_j,_k + s_i + m_iAPR + p_i,_j + \epsilon_i,_j,_k$

```{r}
fullmodel2 = lmer(Pct ~ centeredAPR + (centeredAPR|School) + (1|program.id), REML=T, data=sports)

VCrandom <- VarCorr(fullmodel2)
print(VCrandom, comp = c("Variance", "Std.Dev."))
cat(" Number of Level Two groups = ",
    summary(fullmodel2)$ngrps[1], "\n", 
    "Number of Level Three groups = ",
    summary(fullmodel2)$ngrps[2])
coef(summary(fullmodel2))
```

$Y_i,_j,_k = \beta_0 +\beta_1APR_i,_j,_k + s_i + p_i,_j + m_iAPR + \epsilon_i,_j,_k$

```{r}
fullmodel3 = lmer(Pct ~ centeredAPR + (1|School) + (centeredAPR|program.id), REML=T, data=sports)

VCrandom <- VarCorr(fullmodel3)
print(VCrandom, comp = c("Variance", "Std.Dev."))
cat(" Number of Level Two groups = ",
    summary(fullmodel3)$ngrps[1], "\n", 
    "Number of Level Three groups = ",
    summary(fullmodel3)$ngrps[2])
coef(summary(fullmodel3))
```

```{r}
fullmodel4 = lmer(Pct ~ centeredAPR + (1|School) + (1|program.id) + (1|Years10), REML=T, data=sports)

VCrandom <- VarCorr(fullmodel4)
print(VCrandom, comp = c("Variance", "Std.Dev."))
cat(" Number of Level Two groups = ",
    summary(fullmodel4)$ngrps[1], "\n", 
    "Number of Level Three groups = ",
    summary(fullmodel4)$ngrps[2])
coef(summary(fullmodel4))
```